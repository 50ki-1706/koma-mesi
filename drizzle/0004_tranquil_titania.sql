...
CREATE TABLE `__new_recommendation_categories` (
	...
	CONSTRAINT "recommendation_categories_selection_order_check" CHECK("__new_recommendation_categories"."selection_order" between 1 and 3)
);
--> statement-breakpoint
INSERT INTO `__new_recommendation_categories`("id", "batch_id", "category", "selection_order", "created_at") SELECT "id", "batch_id", "category", "selection_order", "created_at" FROM `recommendation_categories`;--> statement-breakpoint
...
CREATE TABLE `__new_recommendations` (
	...
	`created_at` integer DEFAULT (unixepoch()) NOT NULL,
	FOREIGN KEY (`batch_id`) REFERENCES `recommendation_batches`(`id`) ON UPDATE no action ON DELETE cascade,
	FOREIGN KEY (`recommendation_category_id`) REFERENCES `recommendation_categories`(`id`) ON UPDATE no action ON DELETE cascade,
	...
	CONSTRAINT "recommendations_display_order_check" CHECK("__new_recommendations"."display_order" between 1 and 3)
);
--> statement-breakpoint
