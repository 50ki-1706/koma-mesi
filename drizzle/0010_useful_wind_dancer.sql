...
CREATE TABLE `__new_recommendation_batches` (
	...
	CONSTRAINT "recommendation_batches_status_check" CHECK("__new_recommendation_batches"."status" in (?, ?, ?, ?)),
...
CREATE TABLE `__new_recommendation_categories` (
	...
	CONSTRAINT "recommendation_categories_category_check" CHECK("__new_recommendation_categories"."category" in (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?))
);
--> statement-breakpoint
...
CREATE UNIQUE INDEX `recommendations_category_id_distance_group_unique` ON `recommendations` (`recommendation_category_id`,`distance_group`);