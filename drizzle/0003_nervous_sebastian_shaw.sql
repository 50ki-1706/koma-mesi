...
CREATE TABLE `recommendation_categories` (
	...
	`created_at` integer DEFAULT (unixepoch()) NOT NULL,
	FOREIGN KEY (`batch_id`) REFERENCES `recommendation_batches`(`id`) ON UPDATE no action ON DELETE no action,
...
CREATE TABLE `recommendations` (
	...
	`created_at` integer DEFAULT (unixepoch()) NOT NULL,
	FOREIGN KEY (`batch_id`) REFERENCES `recommendation_batches`(`id`) ON UPDATE no action ON DELETE no action,
	FOREIGN KEY (`recommendation_category_id`) REFERENCES `recommendation_categories`(`id`) ON UPDATE no action ON DELETE no action,
